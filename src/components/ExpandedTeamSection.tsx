                  ...
                  <div className="p-4">
                    <div className="flex flex-wrap gap-2 mb-4">
                      {member.credentials.map((credential, idx) => (
                        <span 
                          key={idx} 
                          className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-[#00d2ff]/15 text-[#00d2ff] border border-[#00d2ff]/30 whitespace-normal hover:bg-[#00d2ff]/20 transition-colors"
                        >
                          {credential}
                        </span>
                      ))}
          ...
          <TabsContent value="detailed">
            <div className="space-y-10">
              {teamMembers.map((member, index) => (
                <motion.div 
                  key={member.id}
                  className="glass-card border border-[#00d2ff]/30 rounded-xl overflow-hidden p-5 md:p-6 md:pb-5"
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                  viewport={{ once: true }}
                >
                  <div className="flex flex-col md:flex-row gap-6">
                    <div className="md:w-1/3 lg:w-1/4 flex-shrink-0">
                      <div className="relative rounded-xl overflow-hidden mb-3 aspect-[3/4] tech-glow">
                        <img 
                          src={member.image} 
                          alt={member.name}
                      ...
                      <p className="text-[#00d2ff] mb-2">{member.role}</p>
                      ...
                      {/* Credentials shown on mobile */}
                      <div className="flex flex-wrap gap-2 mb-4 md:hidden">
                        {member.credentials.map((credential, idx) => (
                          <span 
                            key={idx} 
                            className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-[#00d2ff]/15 text-[#00d2ff] border border-[#00d2ff]/40 whitespace-normal hover:bg-[#00d2ff]/25 transition-colors"
                          >
                            {credential}
                          </span>
                        ))}
                    ...
                    <div className="md:w-2/3 lg:w-3/4">
                      {member.bio.split("\n\n").map((paragraph, i, arr) => (
                        <p 
                          key={i} 
                          className={`text-white/80 ${i === arr.length - 1 ? 'mb-3' : 'mb-4'} leading-relaxed`}
                        >
                          {paragraph}
                        </p>
                      ))}
                      
                      {/* Credentials shown on desktop */}
                      <div className="hidden md:flex flex-wrap gap-2 mt-4 mb-1">
                        {member.credentials.map((credential, idx) => (
                          <span 
                            key={idx} 
                            className="inline-flex items-center px-3.5 py-1.5 rounded-full text-xs font-medium bg-[#00d2ff]/15 text-[#00d2ff] border border-[#00d2ff]/40 whitespace-normal hover:bg-[#00d2ff]/25 transition-colors shadow-sm"
                          >
                            {credential}
                          </span>
                        ))}
